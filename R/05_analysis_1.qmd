...
aug_norm_exprs_00 <- ncbi_joined |> 
  dplyr::select(-names(ncbi_joined)[1],
                -names(ncbi_joined)[4:16]) |>
...
aug_norm_exprs_01 <- aug_norm_exprs_00 |>
  pivot_longer(cols = colnames(aug_norm_exprs_00[3:length(colnames(aug_norm_exprs_00))]),
               names_to = "gene",
               values_to = "expr_level") |>
  mutate(log_2_expr_level = log2(expr_level)) |>
  dplyr::select(-expr_level)

aug_norm_exprs_02 <- aug_norm_exprs_01 |>
  drop_na() |>
  group_by(gene) |>
  nest() |>
  ungroup()
...
aug_norm_exprs_03 <- aug_norm_exprs_02 |>
  group_by(gene) |>
...
aug_norm_exprs_04 <- aug_norm_exprs_03 |>
...
```{r}
significant_labels <- subset(aug_norm_exprs_04 , 
                          is_significant == "yes") |>
  dplyr::select(gene)
