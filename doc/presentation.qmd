...
::: {.column width="60%"}
-   A point in the right column with some code
...
## Methods: Data load and cleaning {.scrollable}
...
flowchart TD
  ...
  U>"get_gene_expr_by_source()"] -->|"transpose_gene_expr()"| V[transposed_placenta_expr]:::red
  U>"get_gene_expr_by_source()"] -->|"transpose_gene_expr()"| W[transposed_maternal_expr]:::red
  U>"get_gene_expr_by_source()"] -->|"transpose_gene_expr()"| X[transposed_cord_expr]:::red
...
## Methods: Data augment {.scrollable}
...
flowchart TD
  ...
  A["ncbi_clean_expression"]:::red -->|"full_join"| J[(ncbi_joined)]
  B["transposed_placenta_expr"]:::red -->|"inner_join"| J[(ncbi_joined)]
  C["transposed_maternal_expr"]:::red -->|"inner_join"| J[(ncbi_joined)]
  D["transposed_cord_expr"]:::red -->|"inner_join"| J[(ncbi_joined)]
  
  E[ncbi_clean_pheno]:::red -->|"mutate(BMI, BMI_class, age_group)"| J[(ncbi_joined)]
  
  J[(ncbi_joined)] -->|"smoking_expr_fc"| F[smoking_fc_placenta]
  J[(ncbi_joined)] -->|"smoking_expr_fc"| G[smoking_fc_maternal]
  J[(ncbi_joined)] -->|"smoking_expr_fc"| H[smoking_fc_cord]
  J[(ncbi_joined)] -->|"smoking_expr_fc"| I[smoking_fc_all]
...
```{r echo=TRUE, eval=TRUE, out.width="100%", fig.align='center'}
options(bitmapType='cairo', reproducible=TRUE, X11.options=list(type='cairo'))

knitr::include_graphics("/results/plots/description_age_boxplot.png", error = FALSE)
```
